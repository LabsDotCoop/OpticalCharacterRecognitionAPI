	...
	try {
		...
		if (is_object($image))
		{
			...
			if (is_file(OCR_PATH . DIRECTORY_SEPARATOR . $file . '.jpeg'))
				$exec = __DIR__ . '/bin/jpegtopnm "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.jpeg" > "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm"';
				@shell_exec($exec);
			if (is_file(OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm'))
			{
				...
				{
					case "raw":
						return shell_exec(__DIR__ . '/bin/gocr -i "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm" -f UTF8');
						break;
					case "html":
						return shell_exec(__DIR__ . '/bin/gocr -i "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm" -f HTML');
						break;
					default:
						$utf8 = shell_exec(__DIR__ . '/bin/gocr -i "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm" -f UTF8');
						$ascii = shell_exec(__DIR__ . '/bin/gocr -i "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm" -f ASCII');
						$ocrdata = shell_exec(__DIR__ . '/bin/gocr -i "'.OCR_PATH . DIRECTORY_SEPARATOR . $file . '.pnm" -f XML');
						break;
				}
			}
